...
export async function cancelMyRide(req, res) {
  const userId = req.user?.id;
  if (!userId) return res.status(401).json({ message: "Non authentifié." });
  ...
  try {
    ...
    if (Number(ride.driver_id) !== Number(userId)) {
      await conn.rollback();
      return res.status(403).json({ message: "Tu ne peux annuler que tes propres trajets." });
    }
    if (ride.status !== "OPEN") {
      await conn.rollback();
    ...
    await conn.execute(
      `UPDATE rides
       SET status = 'CANCELLED', seats_available = seats_total
       WHERE id = ?`,
      [rideId]
    );
    ...
    await conn.rollback();
    return res.status(500).json({ message: "Erreur serveur lors de l'annulation du trajet." });
  } finally {
    conn.release();
  }
}
